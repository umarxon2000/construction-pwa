...
export type ContractType = "labor_only" | "labor_and_materials";
export type ExpenseCategory = "Material" | "Transport" | "Workers" | "Other";
export type AttendanceStatus = "arrived" | "left";
export interface Project { id?: number; clientName: string; location: string; clientPhone: string; totalEarned: number; advances: number; status: ProjectStatus; createdAt: string; finishedAt?: string; contractType?: ContractType; }
...
export interface CustomerPayment { id?: number; projectId: number; amount: number; note: string; date: string; createdAt: string; }
class ConstructionDB extends Dexie {
  projects!: EntityTable<Project, "id">; expenses!: EntityTable<Expense, "id">; workers!: EntityTable<Worker, "id">; attendance!: EntityTable<Attendance, "id">; payments!: EntityTable<WorkerPayment, "id">; customerPayments!: EntityTable<CustomerPayment, "id">;
  constructor() { super("ustaProDB"); this.version(1).stores({ projects: "++id, status, createdAt", expenses: "++id, projectId, category, date", workers: "++id, status, name", attendance: "++id, workerId, projectId, date, status, [workerId+projectId+date]" }); this.version(2).stores({ projects: "++id, status, createdAt", expenses: "++id, projectId, category, date", workers: "++id, status, name", attendance: "++id, workerId, projectId, date, status, [workerId+projectId+date]", payments: "++id, workerId, date" }); this.version(3).stores({ projects: "++id, status, createdAt, contractType", expenses: "++id, projectId, category, date", workers: "++id, status, name", attendance: "++id, workerId, projectId, date, status, [workerId+projectId+date]", payments: "++id, workerId, date", customerPayments: "++id, projectId, date" }); }
}
