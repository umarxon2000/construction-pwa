...
export default function WorkersPage() {
  const { workers, projects, attendance, payments, fetchAll, addWorker, addPayment, checkInWorker, checkOutWorker, editAttendance } = useAppStore();
  ...
  const savePayment = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    await addPayment(Number(form.get("workerId")), Number(form.get("amount")), String(form.get("note")), String(form.get("date")));
    event.currentTarget.reset();
  };
  const dayAttendance = attendance.filter((item) => item.date === date && item.projectId === projectId);

  return <div className="grid gap-5">
    <div><h1 className="text-2xl font-bold">Ishchilar davomatД±</h1><p className="text-sm text-zinc-500">Sana va obyektni tanlang, keyin kelgan ishchilarni belgilang.</p></div>
    ...
    <form onSubmit={addNewWorker} className="grid grid-cols-[1fr_110px_auto] gap-2">
      ...
      <input required min="0" type="number" name="dailyWage" placeholder="Kunlik soвЂm" className="field" />
      <button className="rounded-xl bg-[#FFD700] px-3 font-bold text-black">QoвЂshish</button>
    </form>
    ...
    <div className="grid gap-2 rounded-2xl border border-zinc-200 bg-white p-3 dark:border-[#FFD700]/25 dark:bg-zinc-950">
      ...
      <select value={projectId} onChange={(event) => setProjectId(Number(event.target.value))} className="field">
        ...
        {projects.filter((project) => project.status === "active").map((project) => <option key={project.id} value={project.id}>{project.clientName} вЂ” {project.location}</option>)}
      </select>
    </div>

    <form onSubmit={savePayment} className="grid gap-2 rounded-2xl border border-zinc-200 bg-white p-3 dark:border-[#FFD700]/25 dark:bg-zinc-950">
      <div className="flex items-center gap-2 font-semibold"><WalletCards size={17} className="text-[#FFD700]"/>Ishchiga haq berish</div>
      <select required name="workerId" defaultValue="" className="field"><option value="" disabled>Ishchini tanlang</option>{workers.filter((worker) => worker.status === "active").map((worker) => <option key={worker.id} value={worker.id}>{worker.name}</option>)}</select>
      <div className="grid grid-cols-2 gap-2"><input required name="amount" type="number" min="0" placeholder="Berilgan summa" className="field"/><input name="date" type="date" defaultValue={date} className="field"/></div>
      <input name="note" placeholder="Izoh, masalan: haftalik haq yoki avans" className="field"/>
      <button className="rounded-xl bg-[#FFD700] py-2 font-bold text-black">To‘lovni saqlash</button>
    </form>
    <div className="grid gap-3">
      {workers.filter((worker) => worker.status === "active").map((worker) => {
        const record = dayAttendance.find((item) => item.workerId === worker.id);
        const accumulated = attendance.filter((item) => item.workerId === worker.id).reduce((sum, item) => sum + worker.dailyWage * item.multiplier, 0); const paid = payments.filter((payment) => payment.workerId === worker.id).reduce((sum, payment) => sum + payment.amount, 0);
        return <article key={worker.id} className="rounded-2xl border border-zinc-200 bg-white p-4 dark:border-[#C0C0C0]/25 dark:bg-zinc-950">
          <div className="flex items-start justify-between gap-3"><div><h2 className="font-bold">{worker.name}</h2><p className="text-sm text-zinc-500">Kunlik: {money(worker.dailyWage)}</p></div><p className="text-right text-xs text-zinc-500">Hisoblangan / berilgan<br /><b className="text-sm text-amber-600 dark:text-[#FFD700]">{money(accumulated)} / {money(paid)}</b></p></div>
